...
class DEBUG_EVENT_UNION(Union):
    _fields_ = [
        ("Exception",         EXCEPTION_DEBUG_INFO), # 0x1
        #        ("CreateThread",      CREATE_THREAD_DEBUG_INFO), # 0x2
        #        ("CreateProcessInfo", CREATE_PROCESS_DEBUG_INFO), # 0x3
        #        ("ExitThread",        EXIT_THREAD_DEBUG_INFO), # 0x4
        #        ("ExitProcess",       EXIT_PROCESS_DEBUG_INFO), # 0x5
        #        ("LoadDll",           LOAD_DLL_DEBUG_INFO), # 0x6
        #        ("UnloadDll",         UNLOAD_DLL_DEBUG_INFO), # 0x7
        #        ("DebugString",       OUTPUT_DEBUG_STRING_INFO), # 0x8
        #        ("RipInfo",           RIP_INFO), # 0x9
    ]
...
class DEBUG_EVENT(Structure):
    _fields_ = [
        ("dwDebugEventCode", DWORD), # 用于告知函数WaitForDebugEvent()所捕获到的事件的确切类型
                                     # 该值还决定了u联合体的取值，详细看DEBUG_EVENT_UNION定义注释，前面也有定义相关常量
        ("dwProcessId",      DWORD),
        ("dwThreadId",       DWORD),
        ("u",                DEBUG_EVENT_UNION),
    ]
...
# Used by the WOW64_CONTEXT structure
# refer to https://docs.microsoft.com/en-us/windows/desktop/api/WinNT/ns-winnt-_wow64_context
# for 32-bit
...
# refer to https://docs.microsoft.com/en-us/windows/desktop/api/winnt/ns-winnt-context
# ARM-architecture see https://docs.microsoft.com/en-us/windows/desktop/api/WinNT/ns-winnt-_arm64_nt_context
...
# 实在找不到官方出处了，只能找到这个：
# https://stuff.mit.edu/afs/sipb/project/wine/src/wine-0.9.37/include/winnt.h
